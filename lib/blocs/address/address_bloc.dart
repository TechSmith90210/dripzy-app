  ...
  Future<void> _handleAddAddress(
    ...
    result.when(
      success: (newAddress) {
        ...
        final List<Address> updatedList =
            state.addresses.map((existing) {
      ...
      failure:
          (message) => emit(
            state.copyWith(status: AddressStatus.failure, error: message),
          ),
    );
  }
        ...
        final updated =
            state.addresses.map((address) {
              return address.copyWith(
                isDefault: address.id == updatedDefault.id,
              );
            }).toList();

        emit(state.copyWith(status: AddressStatus.success, addresses: updated));
        emit(state.copyWith(status: AddressStatus.initial));
      },
      failure:
          (error) =>
